...
class TreeObsForRailEnv(ObservationBuilder):
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        root_node_observation = TreeObsForRailEnv.Node(dist_own_target_encountered=0, dist_other_target_encountered=0,
                                                       ...
                                                       time_step=self.env._elapsed_steps/self.env._max_episode_steps, # Divide values to between 0 and 1 to prevent clipping
                                                       agent_release_date=agent.release_date/self.env._max_episode_steps, # Divide values to between 0 and 1 to prevent clipping
                                                       agent_deadline=agent.deadline/self.env._max_episode_steps, # Divide values to between 0 and 1 to prevent clipping
    ...
    def _explore_branch(self, handle, position, direction, tot_dist, depth):
        ...
        node = TreeObsForRailEnv.Node(dist_own_target_encountered=own_target_encountered,
                                      ...
                                      time_step=(self.env._elapsed_steps/self.env._max_episode_steps),      # Divide values to between 0 and 1 to prevent clipping
                                      agent_release_date=(agent.release_date/self.env._max_episode_steps),
                                      agent_deadline=(agent.deadline/self.env._max_episode_steps),
